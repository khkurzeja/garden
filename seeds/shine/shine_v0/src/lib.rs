...
pub async fn run() 
{
    ...
    type Vertex = gfx::SimpleTextVertex;
    ...
    let simple_text_pipeline = gfx::make_simple_text_pipeline(&device, surface_config.format, gfx::SimpleTextVertUniforms::new(), gfx::SimpleTextFragUniforms::new());
    ...
    struct State {
        input: input::Input,
        instances: Vec<gfx::SimpleTextInstance>,
    ...
    let update = |state: &mut State, width: f64, _height: f64|
    {
        ...
        // Scaled by 1
        for i in 0..26 {
            instances.push(gfx::SimpleTextInstance {
                position: [10.0 + i as f32 * 8.0, 100.0],
                scale: 1.0,
                //color: [0.0, 0.0, 0.0],
                color: 0xff0000ff,  // abgr
    ...
    let render = |state: &mut State, width: f32, height: f32| -> Result<(), wgpu::SurfaceError> 
    {
        ...
        gfx::easy_write_buffer(&device, &queue, &mut state.instance_buffer, bytemuck::cast_slice(instances.as_slice()));

        let mut basic_vert_uniforms = gfx::SimpleTextVertUniforms::new();
        {
            let sx = 2.0 / width;
            let sy = -2.0 / height;
            let tx = -1.0;
        ...
        queue.write_buffer(&simple_text_pipeline.vert_uniform_buffer, 0, bytemuck::cast_slice(&[basic_vert_uniforms]));

        let mut basic_frag_uniforms = gfx::SimpleTextFragUniforms::new();
        {
            basic_frag_uniforms.pixel_size = [1.0 / width as f32,  1.0 / height as f32, 420.0, 420.0,];
        }
        queue.write_buffer(&simple_text_pipeline.frag_uniform_buffer, 0, bytemuck::cast_slice(&[basic_frag_uniforms]));
    ...
    
        {
            ...
            render_pass.set_pipeline(&simple_text_pipeline.render_pipeline);
            render_pass.set_bind_group(0, &simple_text_pipeline.uniform_bind_group, &[]);
