...
#[repr(C)]
#[derive(Debug, Copy, Clone, bytemuck::Pod, bytemuck::Zeroable)]
pub struct PanoptigonGridUniforms {
    pub camera: [f32; 12],  // 12 instead of 9, because columns each have an entry of padding.
    pub map_camera: [f32; 12],
    pub screen_to_clip: [f32; 12],
}
impl PanoptigonGridUniforms {
    pub fn new() -> Self {
        Self {
            camera: [0.0; 12],
            map_camera: [0.0; 12],
...
pub struct PanoptigonGridPipeline {
    pub uniform_buffer: Buffer,
    pub uniform_bind_group: BindGroup,
    pub render_pipeline: RenderPipeline,
}

pub fn make_panoptigon_grid_pipeline(device: &wgpu::Device, target_format: TextureFormat, uniforms: PanoptigonGridUniforms) -> PanoptigonGridPipeline
{
    let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: Some("Shader"),
        source: wgpu::ShaderSource::Wgsl(include_str!("shader/panoptigon_grid.wgsl").into()),
    });
